...
def make_env(env_id: str, rank: int, seed: int = 0, wrappers: dict | None = None, **kwargs):
    ...
    def _init():
        ...
        for wrap in wrappers:
            if not (wrap == "MonitorExtended-v0" and rank != 0):
                env = apply_wrapper(env, wrap, wrappers[wrap])
        return env
...
def train_network(envid):
    ...
    make_envs = [
        make_env(env_id=envid, rank=i, seed=config["seed"], wrappers=config["wrappers_kwargs"], **env_kwargs)
    ...
    model = algo(
        ...
        env,
        verbose=0,
        n_steps=config["rollout"],
        n_cpu_tf_sess=config["n_thrds"],
        tensorboard_log=None,
        policy_kwargs={"feature_extraction": "mlp", "n_lstm": config["hidden_size"]},
        **config["alg_kwargs"],
    )
    chckpnt_cllbck = CheckpointCallback(
        save_freq=int(config["num_steps"] / 10), save_path=modelpath, name_prefix="model"
    )
...
def order_by_sufix(file_list):
    file_list = [Path(x).name for x in file_list]
    ...
    sorted_list = [x for _, x in sorted(zip(sfx, file_list))]  # noqa: B905
    if flag:
...
def run_network(envid):
    ...
    if len(files) > 0:
        ...
        algo = get_alg(config["alg"])
        model = algo.load(modelpath / model_name, tensorboard_log=None, custom_objects={"verbose": 0})

        # Environment
        env = make_env(env_id=envid, rank=0, seed=seed, wrappers=wrappers_kwargs, **env_kwargs)()
        ...
        for stp in range(int(num_steps)):
            ...
            action, _states = model.predict(extend_obs(ob, config["n_thrds"]), state=_states, mask=done)
            action = action[0]
