...
def analysis_example_units_by_condition(activity, info, config):
    ...
    for example_id in example_ids:
        ...
        fig, axes = plt.subplots(len(conditions), 1, figsize=(1.2, 0.8 * len(conditions)), sharex=True)
        for i, condition in enumerate(conditions):
            ...
            if i == 0:
                ax.set_title(f"Unit {example_id + 1:d}")


def analysis_pca_by_condition(activity, info):
    ...
    for condition in conditions:
        ...
        ax = fig.add_axes([0.2, 0.2, 0.7, 0.7])  # noqa: F841
        for value in values:
