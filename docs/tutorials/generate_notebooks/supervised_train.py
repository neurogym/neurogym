...
def get_modelpath(envid: str) -> Path:
    # Make a local file directories
    path = Path("files") / envid
...
def train_network(envid):
    ...
    # Save config
    with (modelpath / "config.json").open("w") as f:
        json.dump(config, f)

    # Make supervised dataset
    dataset = ngym.Dataset(envid, env_kwargs=env_kwargs, batch_size=config["batch_size"], seq_len=config["seq_len"])
    env = dataset.env
    act_size = env.action_space.n
    # Train network
    net = Net(input_size=env.observation_space.shape[0], hidden_size=config["hidden_size"], output_size=act_size)
    net = net.to(device)
...
# perf = 0  # noqa: ERA001
# num_trial = 200  # noqa: ERA001
# for i in range(num_trial):
#     env.new_trial()  # noqa: ERA001
#     ob, gt = env.ob, env.gt  # noqa: ERA001
#     ob = ob[:, np.newaxis, :]  # Add batch axis  # noqa: ERA001
#     inputs = torch.from_numpy(ob).type(torch.float).to(device)  # noqa: ERA001
#     action_pred = net(inputs)  # noqa: ERA001
#     action_pred = action_pred.detach().numpy()  # noqa: ERA001
#     action_pred = np.argmax(action_pred, axis=-1)  # noqa: ERA001
#     perf += gt[-1] == action_pred[-1, 0]  # noqa: ERA001
# perf /= num_trial  # noqa: ERA001
# print('Average performance in {:d} trials'.format(num_trial))  # noqa: ERA001
# print(perf)  # noqa: ERA001


def infer_test_timing(env):
    """Infer timing of environment for testing."""
    timing = {}
    for period in env.timing:
...
def run_network(envid):
    ...
    """
    modelpath = get_modelpath(envid)
    with (modelpath / "config.json").open() as f:
        config = json.load(f)
    ...
    with torch.no_grad():
        net = Net(
            input_size=env.observation_space.shape[0], hidden_size=config["hidden_size"], output_size=env.action_space.n
        )
        net = net.to(device)
