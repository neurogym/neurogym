...
# TODO: wrappers from neurogym/wrappers/block.py are not being tested.
...
def test_passreward():
    ...
    with warnings.catch_warnings():
        ...
        for _ in range(num_steps):
            ...
            assert obs[-1] == rew, "Previous action is not part of observation"
            if terminated:
                env.reset()
...
def test_reactiontime():
    ...
    with warnings.catch_warnings():
        ...
        num_steps = 200
        urgency = -0.1  # value added to the reward
        ths = [-0.5, 0.5]  # threholds to make a decision
        ...
        for i in range(num_steps):
            ...
            if info["new_trial"]:
                ...
                steps_in_trial += 1
                assert not end_of_trial, (  # TODO: the error seems to be unrelated to the wrapper.
                    f"Trial did not end after decision ({action}) on iteration {i}, step {steps_in_trial}."
                )
                obs_cum += obs[1] - obs[2]
...
def test_noise():
    ...
    with warnings.catch_warnings():
        ...
        env = make("PerceptualDecisionMaking-v0", **env_args)
        # FIXME: perf_th is not (no longer?) used in `Noise` wrapper, causing the fail.
        env = Noise(env)  # , perf_th=target_perf)
        env.reset()
        perf = []
        std_noise = 0
        for _ in range(1000):
            ...
            if info["new_trial"]:
                perf.append(info["perf"])  # info does not contain `perf` key
            if terminated:
                env.reset()
